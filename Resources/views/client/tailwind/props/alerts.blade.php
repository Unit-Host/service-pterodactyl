...
@if(ErrorLog::where('order_id', $order->id)->where('severity','!=','RESOLVED')->count() !== 0)
    <div id="alert-additional-content-2" class="p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
        ...
        <svg aria-hidden="true" class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
        <span class="sr-only">Important</span>
        <h3 class="text-lg font-medium">Failed to generate your server</h3>
        </div>
        <div class="mt-2 mb-4 text-sm">
            We attempted to create a server for you, however something went wrong. Please contact an Administrator to check order from admin area for more detailed logs.
        </div>
        <div class="flex">
        </div>
    </div>
