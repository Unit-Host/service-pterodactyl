    ...
    <div class="relative w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 space-y-6">
                ...
                @endif
                <form action="{{ route('service', ['order' => $order->id, 'page' => 'cancel-service']) }}"
                      method="POST">
                    @csrf
                    <div class="flex mb-4">
                        ...
                        <div class="ml-2 text-sm">
                            <label for="helper-radio"
                                ...
                                {!! __('client.cancel_at_end_of_term') !!}
                            </label>
                    ...
                    <div class="flex mb-6">
                        ...
                        <div class="ml-2 text-sm">
                            <label for="helper-radio"
                                   class="font-medium text-gray-900 dark:text-gray-300">
                                {!! __('client.cancel_immediately') !!}
                            </label>
                            <p id="helper-radio-text"
                               class="text-xs font-normal text-gray-500 dark:text-gray-300">
                                {!! __('client.cancel_immediately_desc') !!}
                        </div>
                    </div>
                    <div class="flex flex-col">
                        <label for="message"
                               class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            {!! __('client.cancellation_reason') !!}
                        </label>
                        ...
                        @if($order->price['cancellation_fee'] > 0)
                            <label for="gateway"
                                ...
                                {!! __('client.payment_method_reason') !!}
                            </label>
                            <select
                                ...
                                @foreach (App\Models\Gateways\Gateway::getActive() as $gateway)
                                    @if($gateway->name == 'Balance')
                                        <option value="{{ $gateway->id }}"
                                                @if(Auth::user()->balance >= $order->price['cancellation_fee']) selected @endif>
